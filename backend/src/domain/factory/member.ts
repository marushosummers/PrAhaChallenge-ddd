...
export class MemberFactory {
  public static async create(params: { name: string, email: string, taskRepo: ITaskRepository }): Promise<Member> {
...
export class MemberTaskFactory {
  public static create(params: { taskId: string }): MemberTask {
    const { taskId } = params
    return new MemberTask({ id: createRandomIdString(), taskId: taskId, progressStatus: 'NOTYET' }); // Taskは未着手で生成される
  }
}
